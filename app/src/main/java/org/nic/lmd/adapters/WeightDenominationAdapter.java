...
public class WeightDenominationAdapter extends BaseAdapter {
    ...
    public WeightDenominationAdapter(Activity activity, String cot_id, String val_year, TextView textView) {
        ...
        if (this.cot_id.equals("0"))
            denomintionEntities = new DataBaseHelper(activity).getWeightDenomination(this.cot_id, "0");
        else
            denomintionEntities = new DataBaseHelper(activity).getWeightDenomination(this.cot_id, "1");

    }

    public WeightDenominationAdapter(Activity activity, ArrayList<DenomintionEntity> denomintionEntities) {
        this.activity = activity;
        this.denomintionEntities = denomintionEntities;
        layoutInflater = this.activity.getLayoutInflater();
    }
