...
import org.nic.lmd.entities.ThanaEntity;
...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    public long saveSubDivision(ArrayList<SubDivision> res) {
        ...
        try {
            for (SubDivision typeEntity : res) {
                ContentValues values = new ContentValues();
                values.put("subDivId",String.valueOf(typeEntity.getId()).trim());
                values.put("subDivName", String.valueOf(typeEntity.getSub_div_name()).trim());
                values.put("distId", String.valueOf(typeEntity.getDist_id()).trim());
                String[] whereArgs = new String[]{String.valueOf(typeEntity.getId()).trim()};
    ...
    public ArrayList<SubDivision> getSubDivision(String subdiv_id) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                subDivision.setId(Integer.parseInt(cursor.getString(cursor.getColumnIndex("subDivId"))));
                subDivision.setSub_div_name(cursor.getString(cursor.getColumnIndex("subDivName")));
                subDivision.setDist_id(Integer.parseInt(cursor.getString(cursor.getColumnIndex("distId"))));
    ...
    public long saveVenderJsonData(String res) {
        ...
        try {
            ...
            for (int j = 0; j < jsonArray3.length(); j++) {
                ...
                //instrumentEntity.setIns_class(getSelectedInsClass(String.valueOf(cursor.getInt(cursor.getColumnIndex("_id")))));
    ...
    public long saveMarketInspectionTabs(ArrayList<MarketInspectionTab> marketInspectionTabs) {
        long c = -1;
        SQLiteDatabase db = null;
        try {
            for (MarketInspectionTab marketInspectionTab : marketInspectionTabs) {
                db = getWritableDatabase();
                ContentValues values = new ContentValues();
                values.put("market_ins_id",marketInspectionTab.getMarket_ins_id());
                values.put("name", marketInspectionTab.getName());
                values.put("value", marketInspectionTab.getValue());
                c = db.update("MarketInspectionTab", values, "market_ins_id=?", new String[]{String.valueOf(marketInspectionTab.getMarket_ins_id())});
                if (c <= 0) {
                c = db.insert("MarketInspectionTab", null, values);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ArrayList<MarketInspectionTab> getMarketInspectionTabs() {
        ArrayList<MarketInspectionTab> marketInspectionTabs = new ArrayList<>();
        Cursor cursor = null;
        SQLiteDatabase db = null;
        String quary = "select * from MarketInspectionTab";
        try {
            db = getReadableDatabase();
            cursor = db.rawQuery(quary, null);
            while (cursor.moveToNext()) {
                MarketInspectionTab marketInspectionTab = new MarketInspectionTab();
                marketInspectionTab.setMarket_ins_id(cursor.getInt(cursor.getColumnIndex("market_ins_id")));
                marketInspectionTab.setName(cursor.getString(cursor.getColumnIndex("name")));
                marketInspectionTab.setValue( cursor.getString(cursor.getColumnIndex("value")));
                marketInspectionTabs.add(marketInspectionTab);
            }
        } catch (Exception e) {
            e.printStackTrace();
            Log.e("error", "Exception in getting MarketInspectionTab");
            return marketInspectionTabs;
